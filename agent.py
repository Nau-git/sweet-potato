...
try:
    ...
    # print("query result : ",db.run("SELECT * FROM Artist LIMIT 10;"))
except:
    print("DB is not ready to use.")


"""
Define a few utility functions to help with the agent implementation. 
...
"""
Few tools that the agent will use to interact with the database.
...
# toolkit = SQLDatabaseToolkit(db=db, llm=ChatOpenAI(model="gpt-4o-mini", api_key=os.environ.get('OPENAI_API_KEY')))
...
while True:
    ...
    if user_input.lower() == "exit":
        exit()
    responses = app.invoke({"messages": [("user", user_input)]})
    final_ans = responses["messages"][-1].tool_calls[0]["args"]["final_answer"]
    print("Agent: ",final_ans)
